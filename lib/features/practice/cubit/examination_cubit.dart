...
class ExaminationCubit extends Cubit<ExaminationState> {
  ...
  ExaminationCubit(this.authenticationRepository, this.examinationRepository) : super(const ExaminationState.loading()){
  ...
  void chooseAnswer(String newSelected, int questionId){
    _choices?.where((choice) => choice.id == questionId).first.selected = newSelected;
  }
  ...
  Future<void> submitExamination() async{
    ...
    try{
      emit(const ExaminationState.loading());
      _examination = await examinationRepository.submitExamination(examination!.id!, data);
      examinationRepository.examination = _examination;
...
class ExaminationState with _$ExaminationState {
  ...
  const factory ExaminationState.loaded(Test data, List<Choice> selections) = ExaminationStateLoaded;
